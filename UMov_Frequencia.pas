...
uses UDM, UMov_Frequencia_FaltasDia_QR, UMov_Frequencia_FaltasAluno_QR,
  UMenu, UDM2, UMov_Frequencia_FaltasPeriodo_QR;
...
Begin
  ...
  for i := 0 to NumLinha - 1 do Begin
     ...
     TabelaFrequenciaData.Value := StrToDate(copy(Linha,1,10));
     TabelaFrequenciaHora.Value := StrToTime(copy(Linha,12,5));
     TabelaFrequenciaCodigo.Value := StrToInt(copy(Linha,27,5));
  ...
  for i := 0 to NumLinha - 1 do
   ...
   Begin
    ...
    //     Linha := Texto.Lines[i];
         ...
         dm.Faltas.ParamByName('bcod_alu').Value := strToInt(Copy(Linha,27,5));
         dm.Faltas.ParamByName('w_data').Value := StrtoDate(Copy(Linha,1,10)); // para comparar com a data o Alvaro tem que fazer a chamada todos os dias.
         dm.Faltas.Open;
         if (not dm.FaltasALU_CODIGO.IsNull) Then
         Begin
           dm2.iqpresenca.Insert;
           dm2.iqpresencaDATA.Value := StrToDate(Copy(Linha,1,10));
           dm2.iqpresencaHORA.Value := StrToTime(Copy(Linha,12,5));
           dm2.iqpresencaALU_PRESENCA.Value  := strToInt(Copy(Linha,27,5));
           dm2.iqpresenca.Post;
           Gauge.Progress := i;
         End;
         dm.Faltas.Close;
