...
begin
  ...
  Exame := false;
  N_Exames := 0;
    if FileExists('G:\Fotos\'+ IntToStr(DM.BoletimRCOD_ALU.Value)+'.jpg') or  FileExists('G:\Fotos\'+ IntToStr(DM.BoletimRCOD_ALU.Value)+'.JPG') then
...
begin
  ...
  if DM.BoletimRACIMA_MEDIA.Value = 'N' then begin
    inc(DiscAbaixo);
    Exame := true;
  if dm.boletimRMODALIDADE.Value = 'DME' Then
  Begin
     dec(DiscAbaixo);
  //   inc(DiscAcima);
  End;
 end;
  if (((DM.BoletimRM1.IsNull) or (DM.BoletimRM2.IsNull) or
     (DM.BoletimRM3.IsNull) or (DM.BoletimRM4.IsNull)) and (dm.BoletimRDIS_CODIGO.Value <> 21)) then
     Cursando := True;
...
begin
  ...
  if Exame then
    if N_Exames = 0 then Resultado.Caption := 'Exame'
     else
