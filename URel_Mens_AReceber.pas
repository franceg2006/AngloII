 ...
 begin
  Try
   ...
   if Turma.KeyValue = null then begin
     Rel_QRMens_AReceber.Bloquetos.SQL.Strings[11] := '';
     Rel_QRMens_AReceber.TurmaX.Caption := 'GERAL';
     end
    else begin
     Rel_QRMens_AReceber.Bloquetos.SQL.Strings[11] := 'and (A.SERIE = :serie) and (A.TURMA = :turma)';
   ...
   if Ordem.ItemIndex = 0 then begin
     Rel_QRMens_AReceber.Bloquetos.SQL.Strings[12] := 'order by B.VENCIMENTO, A.NOME, B.NOSSO_NUMERO';
     Rel_QRMens_AReceber.QRGroup1.Enabled := true;
     Rel_QRMens_AReceber.DetailBand1.Enabled := true;
    end
   else begin
     Rel_QRMens_AReceber.Bloquetos.SQL.Strings[12] := 'order by A.NOME';
