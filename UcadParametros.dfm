object CadParametros: TCadParametros
  Left = 493
  Top = 144
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Parâmetros'
  ClientHeight = 523
  ClientWidth = 595
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PageControl1: TPageControl
    Left = 0
    Top = 0
    Width = 595
    Height = 593
    ActivePage = TabSheet1
    Align = alTop
    TabOrder = 0
    object TabSheet1: TTabSheet
      Caption = 'Bloquetos'
      object Label1: TLabel
        Left = 1
        Top = 13
        Width = 93
        Height = 13
        Alignment = taRightJustify
        Caption = 'Origem Alunos :'
      end
      object Label2: TLabel
        Left = 5
        Top = 64
        Width = 89
        Height = 13
        Alignment = taRightJustify
        Caption = 'Próx.Bloqueto :'
        FocusControl = DBEdit1
      end
      object Label5: TLabel
        Left = 5
        Top = 134
        Width = 129
        Height = 13
        Caption = 'Mensagem / Bloqueto:'
      end
      object Label8: TLabel
        Left = 5
        Top = 93
        Width = 187
        Height = 13
        Caption = 'Local de Pagamento /  Bloqueto:'
      end
      object Label3: TLabel
        Left = 400
        Top = 88
        Width = 101
        Height = 13
        Alignment = taRightJustify
        Caption = 'Dia Vcto Padrão :'
        FocusControl = DBEdit6
      end
      object Label4: TLabel
        Left = 12
        Top = 283
        Width = 56
        Height = 13
        Alignment = taRightJustify
        Caption = '1ª Série :'
        FocusControl = DBEdit7
      end
      object Label6: TLabel
        Left = 12
        Top = 307
        Width = 56
        Height = 13
        Alignment = taRightJustify
        Caption = '2ª Série :'
        FocusControl = DBEdit8
      end
      object Label7: TLabel
        Left = 12
        Top = 331
        Width = 56
        Height = 13
        Alignment = taRightJustify
        Caption = '3ª Série :'
        FocusControl = DBEdit9
      end
      object Label9: TLabel
        Left = 75
        Top = 263
        Width = 70
        Height = 13
        Alignment = taRightJustify
        Caption = 'Mensalidade'
        FocusControl = DBEdit6
      end
      object Label10: TLabel
        Left = 176
        Top = 263
        Width = 45
        Height = 13
        Alignment = taRightJustify
        Caption = 'Material'
        FocusControl = DBEdit6
      end
      object Label11: TLabel
        Left = 3
        Top = 259
        Width = 57
        Height = 13
        Alignment = taRightJustify
        BiDiMode = bdLeftToRight
        Caption = 'VALORES'
        FocusControl = DBEdit6
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentFont = False
      end
      object Label12: TLabel
        Left = 457
        Top = 267
        Width = 116
        Height = 13
        Alignment = taRightJustify
        BiDiMode = bdLeftToRight
        Caption = 'Margens Bloqueto'
        FocusControl = DBEdit6
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentFont = False
      end
      object Label13: TLabel
        Left = 464
        Top = 291
        Width = 58
        Height = 13
        Alignment = taRightJustify
        Caption = 'Superior :'
        FocusControl = DBEdit6
      end
      object Label15: TLabel
        Left = 460
        Top = 315
        Width = 62
        Height = 13
        Alignment = taRightJustify
        Caption = 'Esquerda :'
        FocusControl = DBEdit6
      end
      object Label14: TLabel
        Left = 6
        Top = 37
        Width = 88
        Height = 13
        Alignment = taRightJustify
        Caption = 'Diretório SCA :'
      end
      object Gauge1: TGauge
        Left = 460
        Top = 13
        Width = 73
        Height = 15
        Progress = 0
        Visible = False
      end
      object ArqX: TLabel
        Left = 461
        Top = -1
        Width = 31
        Height = 13
        Caption = 'ArqX'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Visible = False
      end
      object Label18: TLabel
        Left = 286
        Top = 256
        Width = 54
        Height = 26
        Alignment = taCenter
        Caption = 'Recup.'#13#10'Bimestral'
        FocusControl = DBEdit6
      end
      object Label29: TLabel
        Left = 236
        Top = 263
        Width = 45
        Height = 13
        Alignment = taRightJustify
        Caption = 'Até Mês'
        FocusControl = DBEdit6
      end
      object Label21: TLabel
        Left = 364
        Top = 255
        Width = 77
        Height = 26
        Caption = 'Desc.'#13#10'Pontualidade.'
      end
      object Label33: TLabel
        Left = 321
        Top = 283
        Width = 41
        Height = 26
        Caption = '1ª e 2ª'#13#10'Séries'
      end
      object Label34: TLabel
        Left = 317
        Top = 332
        Width = 47
        Height = 13
        Caption = '3ª Série'
      end
      object Label35: TLabel
        Left = 406
        Top = 294
        Width = 14
        Height = 13
        Caption = '%'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label36: TLabel
        Left = 406
        Top = 334
        Width = 14
        Height = 13
        Caption = '%'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label37: TLabel
        Left = 362
        Top = 64
        Width = 141
        Height = 13
        Caption = 'Dia Vcto Mês Fevereiro :'
      end
      object Label38: TLabel
        Left = 8
        Top = 358
        Width = 60
        Height = 13
        Caption = 'Cursinho :'
      end
      object lbl1: TLabel
        Left = 163
        Top = 397
        Width = 122
        Height = 13
        Caption = 'Valor Matricula Semi:'
      end
      object lbl2: TLabel
        Left = 72
        Top = 376
        Width = 77
        Height = 13
        Caption = 'Pontualidade.'
      end
      object Label39: TLabel
        Left = 11
        Top = 394
        Width = 56
        Height = 13
        Alignment = taRightJustify
        Caption = '1ª Série :'
        FocusControl = DBEdit7
      end
      object Label40: TLabel
        Left = 11
        Top = 418
        Width = 56
        Height = 13
        Alignment = taRightJustify
        Caption = '2ª Série :'
        FocusControl = DBEdit8
      end
      object Label41: TLabel
        Left = 11
        Top = 442
        Width = 56
        Height = 13
        Alignment = taRightJustify
        Caption = '3ª Série :'
        FocusControl = DBEdit9
      end
      object Label42: TLabel
        Left = 8
        Top = 470
        Width = 60
        Height = 13
        Caption = 'Cursinho :'
      end
      object FilenameEdit1: TFilenameEdit
        Left = 101
        Top = 8
        Width = 339
        Height = 21
        Filter = 'Arquivos DBF (*.dbf)|*.dbf'
        InitialDir = 'c:\'
        DialogTitle = 'Selecione arquivo original de Alunos'
        DirectInput = False
        NumGlyphs = 1
        TabOrder = 0
      end
      object DBEdit1: TDBEdit
        Left = 101
        Top = 59
        Width = 64
        Height = 21
        DataField = 'PROXIMO_BLOQUETO_I'
        DataSource = DM2.dsParametros_B
        Enabled = False
        TabOrder = 1
      end
      object DBEdit2: TDBEdit
        Left = 5
        Top = 107
        Width = 484
        Height = 21
        DataField = 'LOCALPAG'
        DataSource = DM.dsParametros
        TabOrder = 2
      end
      object DBEdit3: TDBEdit
        Left = 5
        Top = 148
        Width = 484
        Height = 21
        DataField = 'MENSAGEM1'
        DataSource = DM.dsParametros
        TabOrder = 3
      end
      object DBEdit4: TDBEdit
        Left = 5
        Top = 169
        Width = 484
        Height = 21
        DataField = 'MENSAGEM2'
        DataSource = DM.dsParametros
        TabOrder = 4
      end
      object DBEdit5: TDBEdit
        Left = 5
        Top = 235
        Width = 484
        Height = 21
        DataField = 'MENSAGEMSACADO'
        DataSource = DM.dsParametros
        TabOrder = 5
      end
      object DBEdit6: TDBEdit
        Left = 508
        Top = 85
        Width = 35
        Height = 21
        DataField = 'VCTO_PADRAO'
        DataSource = DM.dsParametros
        TabOrder = 6
      end
      object DBEdit7: TDBEdit
        Left = 73
        Top = 278
        Width = 74
        Height = 21
        DataField = 'MENSALIDADE_1'
        DataSource = DM.dsParametros
        TabOrder = 7
      end
      object DBEdit8: TDBEdit
        Left = 73
        Top = 302
        Width = 74
        Height = 21
        DataField = 'MENSALIDADE_2'
        DataSource = DM.dsParametros
        TabOrder = 8
      end
      object DBEdit9: TDBEdit
        Left = 73
        Top = 326
        Width = 74
        Height = 21
        DataField = 'MENSALIDADE_3'
        DataSource = DM.dsParametros
        TabOrder = 9
      end
      object DBEdit10: TDBEdit
        Left = 161
        Top = 278
        Width = 74
        Height = 21
        DataField = 'MATERIAL_1'
        DataSource = DM.dsParametros
        TabOrder = 10
      end
      object DBEdit11: TDBEdit
        Left = 161
        Top = 302
        Width = 74
        Height = 21
        DataField = 'MATERIAL_2'
        DataSource = DM.dsParametros
        TabOrder = 11
      end
      object DBEdit12: TDBEdit
        Left = 161
        Top = 326
        Width = 74
        Height = 21
        DataField = 'MATERIAL_3'
        DataSource = DM.dsParametros
        TabOrder = 12
      end
      object DBEdit13: TDBEdit
        Left = 528
        Top = 286
        Width = 27
        Height = 21
        DataField = 'BLQ_TOP'
        DataSource = DM.dsParametros
        TabOrder = 21
      end
      object DBEdit15: TDBEdit
        Left = 528
        Top = 310
        Width = 27
        Height = 21
        DataField = 'BLQ_LEFT'
        DataSource = DM.dsParametros
        TabOrder = 22
      end
      object DirectoryEdit1: TDirectoryEdit
        Left = 101
        Top = 32
        Width = 304
        Height = 21
        DialogText = 'Selecione diretório do SCA original / ANO'
        DirectInput = False
        NumGlyphs = 1
        ButtonWidth = 20
        TabOrder = 16
      end
      object Atualiza: TBitBtn
        Left = 410
        Top = 32
        Width = 132
        Height = 21
        Caption = 'Atualiza Arquivos'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 17
        OnClick = AtualizaClick
        Glyph.Data = {
          
          3333333333777333333333333333333333333333333333333333}
        NumGlyphs = 2
      end
      object CheckBox1: TCheckBox
        Left = 288
        Top = 283
        Width = 25
        Height = 17
        TabOrder = 18
      end
      object CheckBox2: TCheckBox
        Left = 288
        Top = 307
        Width = 25
        Height = 17
        TabOrder = 19
      end
      object CheckBox3: TCheckBox
        Left = 288
        Top = 331
        Width = 24
        Height = 17
        TabOrder = 20
      end
      object DBEdit22: TDBEdit
        Left = 241
        Top = 326
        Width = 29
        Height = 21
        DataField = 'MES_MATERIAL3'
        DataSource = DM.dsParametros
        TabOrder = 15
      end
      object DBEdit21: TDBEdit
        Left = 241
        Top = 302
        Width = 29
        Height = 21
        DataField = 'MES_MATERIAL2'
        DataSource = DM.dsParametros
        TabOrder = 14
      end
      object DBEdit20: TDBEdit
        Left = 241
        Top = 278
        Width = 29
        Height = 21
        DataField = 'MES_MATERIAL1'
        DataSource = DM.dsParametros
        TabOrder = 13
      end
      object DBEdit17: TDBEdit
        Left = 370
        Top = 286
        Width = 34
        Height = 21
        DataField = 'REAJUSTE_1_2'
        DataSource = DM.dsParametros
        TabOrder = 23
      end
      object DBEdit18: TDBEdit
        Left = 370
        Top = 326
        Width = 33
        Height = 21
        DataField = 'REAJUSTE_3'
        DataSource = DM.dsParametros
        TabOrder = 24
      end
      object DBEdit27: TDBEdit
        Left = 5
        Top = 191
        Width = 484
        Height = 21
        DataField = 'MENSAGEM3'
        DataSource = DM.dsParametros
        TabOrder = 25
      end
      object DBEdit28: TDBEdit
        Left = 5
        Top = 213
        Width = 484
        Height = 21
        DataField = 'MENSAGEM4'
        DataSource = DM.dsParametros
        TabOrder = 26
      end
      object DBEdit29: TDBEdit
        Left = 508
        Top = 62
        Width = 36
        Height = 21
        DataField = 'VCTOFEVEREIRO'
        DataSource = DM.dsParametros
        TabOrder = 27
      end
      object valorc: TDBEdit
        Left = 73
        Top = 352
        Width = 74
        Height = 21
        DataField = 'MENSALIDADE_4'
        DataSource = DM.dsParametros
        TabOrder = 28
      end
      object materialc: TDBEdit
        Left = 161
        Top = 352
        Width = 74
        Height = 21
        DataField = 'MATERIAL_4'
        DataSource = DM.dsParametros
        TabOrder = 29
      end
      object ateomesc: TDBEdit
        Left = 241
        Top = 352
        Width = 29
        Height = 21
        DataField = 'MES_MATERIAL4'
        DataSource = DM.dsParametros
        TabOrder = 30
      end
      object dbedt1: TDBEdit
        Left = 293
        Top = 389
        Width = 87
        Height = 21
        DataField = 'MENSALIDADE_5'
        DataSource = DM.dsParametros
        TabOrder = 31
      end
      object DBEdit30: TDBEdit
        Left = 73
        Top = 390
        Width = 74
        Height = 21
        DataField = 'VALOR_PONTUALIDADE1'
        DataSource = DM.dsParametros
        TabOrder = 32
      end
      object DBEdit31: TDBEdit
        Left = 73
        Top = 414
        Width = 74
        Height = 21
        DataField = 'VALOR_PONTUALIDADE2'
        DataSource = DM.dsParametros
        TabOrder = 33
      end
      object DBEdit32: TDBEdit
        Left = 73
        Top = 438
        Width = 74
        Height = 21
        DataField = 'VALOR_PONTUALIDADE3'
        DataSource = DM.dsParametros
        TabOrder = 34
      end
      object DBEdit33: TDBEdit
        Left = 73
        Top = 464
        Width = 74
        Height = 21
        DataField = 'VALOR_PONTUALIDADE4'
        DataSource = DM.dsParametros
        TabOrder = 35
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'Diversos'
      ImageIndex = 1
      object Label16: TLabel
        Left = 53
        Top = 24
        Width = 46
        Height = 13
        Alignment = taRightJustify
        Caption = 'Média :'
        FocusControl = DBEdit14
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label23: TLabel
        Left = 161
        Top = 45
        Width = 67
        Height = 13
        Alignment = taRightJustify
        Caption = 'Tempo (s) :'
        FocusControl = DBEdit19
      end
      object Label22: TLabel
        Left = 160
        Top = 24
        Width = 110
        Height = 13
        Alignment = taRightJustify
        BiDiMode = bdLeftToRight
        Caption = 'TEMPO / CARTÃO'
        FocusControl = DBEdit6
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentFont = False
      end
      object Label28: TLabel
        Left = 19
        Top = 69
        Width = 108
        Height = 13
        Alignment = taRightJustify
        BiDiMode = bdLeftToRight
        Caption = 'COR / ZEBRADO :'
        FocusControl = DBEdit6
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentFont = False
      end
      object Shape1: TShape
        Left = 131
        Top = 88
        Width = 137
        Height = 52
        Pen.Style = psClear
      end
      object Shape2: TShape
        Left = 132
        Top = 89
        Width = 135
        Height = 10
        Brush.Color = clSilver
        Pen.Style = psClear
      end
      object Shape3: TShape
        Left = 132
        Top = 105
        Width = 135
        Height = 10
        Brush.Color = clSilver
        Pen.Style = psClear
      end
      object Shape4: TShape
        Left = 132
        Top = 121
        Width = 135
        Height = 10
        Brush.Color = clSilver
        Pen.Style = psClear
      end
      object Label20: TLabel
        Left = 5
        Top = 48
        Width = 94
        Height = 13
        Alignment = taRightJustify
        Caption = 'Média Exame :'
        FocusControl = DBEdit16
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Shape5: TShape
        Left = 311
        Top = 168
        Width = 225
        Height = 41
        Brush.Color = clBtnFace
      end
      object DBEdit14: TDBEdit
        Left = 104
        Top = 19
        Width = 35
        Height = 21
        DataField = 'MEDIA'
        DataSource = DM.dsParametros
        Enabled = False
        TabOrder = 0
      end
      object DBEdit19: TDBEdit
        Left = 234
        Top = 40
        Width = 39
        Height = 21
        DataField = 'TEMPO_CARTAO'
        DataSource = DM.dsParametros
        TabOrder = 2
      end
      object MudaCor: TBitBtn
        Left = 19
        Top = 88
        Width = 105
        Height = 25
        Caption = '&Trocar Cor'
        TabOrder = 3
        OnClick = MudaCorClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
      object GroupBox1: TGroupBox
        Left = 311
        Top = 8
        Width = 250
        Height = 73
        Caption = 'Som OK'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 4
        object Label25: TLabel
          Left = 11
          Top = 21
          Width = 135
          Height = 13
          Alignment = taRightJustify
          Caption = 'Frequência (100-5000):'
          FocusControl = DBEdit23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
        end
        object Label24: TLabel
          Left = 25
          Top = 45
          Width = 121
          Height = 13
          Alignment = taRightJustify
          Caption = 'Duração (100-1000):'
          FocusControl = DBEdit24
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit23: TDBEdit
          Left = 155
          Top = 16
          Width = 39
          Height = 21
          DataField = 'SOM_OK_FREQUENCIA'
          DataSource = DM.dsParametros
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object DBEdit24: TDBEdit
          Left = 155
          Top = 40
          Width = 39
          Height = 21
          DataField = 'SOM_OK_DURACAO'
          DataSource = DM.dsParametros
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object BitBtn2: TBitBtn
          Left = 200
          Top = 16
          Width = 40
          Height = 45
          Caption = 'Testa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          OnClick = BitBtn2Click
        end
      end
      object GroupBox2: TGroupBox
        Left = 311
        Top = 88
        Width = 250
        Height = 73
        Caption = 'Som ERRO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 5
        object Label26: TLabel
          Left = 11
          Top = 21
          Width = 135
          Height = 13
          Alignment = taRightJustify
          Caption = 'Frequência (100-5000):'
          FocusControl = DBEdit25
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
        end
        object Label27: TLabel
          Left = 25
          Top = 45
          Width = 121
          Height = 13
          Alignment = taRightJustify
          Caption = 'Duração (100-1000):'
          FocusControl = DBEdit26
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit25: TDBEdit
          Left = 155
          Top = 16
          Width = 39
          Height = 21
          DataField = 'SOM_ERRO_FREQUENCIA'
          DataSource = DM.dsParametros
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object DBEdit26: TDBEdit
          Left = 155
          Top = 40
          Width = 39
          Height = 21
          DataField = 'SOM_ERRO_DURACAO'
          DataSource = DM.dsParametros
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object BitBtn3: TBitBtn
          Left = 200
          Top = 16
          Width = 40
          Height = 45
          Caption = 'Testa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          OnClick = BitBtn3Click
        end
      end
      object Panel1: TPanel
        Left = 16
        Top = 152
        Width = 110
        Height = 151
        Enabled = False
        TabOrder = 6
        object Label17: TLabel
          Left = 16
          Top = 8
          Width = 78
          Height = 39
          Alignment = taCenter
          Caption = 'Distribuição'#13#10'de Alunos'#13#10'nas Turmas'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Gauge2: TGauge
          Left = 5
          Top = 132
          Width = 100
          Height = 14
          Progress = 0
          Visible = False
        end
        object Dist1: TBitBtn
          Left = 16
          Top = 56
          Width = 75
          Height = 25
          Caption = '1ª Série'
          TabOrder = 0
          OnClick = Dist1Click
        end
        object Dist2: TBitBtn
          Left = 16
          Top = 80
          Width = 75
          Height = 25
          Caption = '2ª Série'
          TabOrder = 1
          OnClick = Dist2Click
        end
        object Dist3: TBitBtn
          Left = 16
          Top = 104
          Width = 75
          Height = 25
          Caption = '3ª Série'
          TabOrder = 2
          OnClick = Dist3Click
        end
      end
      object Panel2: TPanel
        Left = 126
        Top = 152
        Width = 110
        Height = 151
        Enabled = False
        TabOrder = 7
        object Label19: TLabel
          Left = 8
          Top = 8
          Width = 96
          Height = 39
          Alignment = taCenter
          Caption = 'Reorganização'#13#10'dos Números'#13#10'nas Turmas'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Gauge3: TGauge
          Left = 5
          Top = 132
          Width = 100
          Height = 14
          Progress = 0
          Visible = False
        end
        object Reorg1: TBitBtn
          Left = 16
          Top = 56
          Width = 75
          Height = 25
          Caption = '1ª Série'
          TabOrder = 0
          OnClick = Reorg1Click
        end
        object Reorg2: TBitBtn
          Left = 16
          Top = 80
          Width = 75
          Height = 25
          Caption = '2ª Série'
          TabOrder = 1
          OnClick = Reorg2Click
        end
        object Reorg3: TBitBtn
          Left = 16
          Top = 104
          Width = 75
          Height = 25
          Caption = '3ª Série'
          TabOrder = 2
          OnClick = Reorg3Click
        end
      end
      object DBEdit16: TDBEdit
        Left = 104
        Top = 43
        Width = 35
        Height = 21
        DataField = 'MEDIA_EXAME'
        DataSource = DM.dsParametros
        Enabled = False
        TabOrder = 1
      end
      object notasnet: TDBCheckBox
        Left = 319
        Top = 184
        Width = 209
        Height = 17
        DataField = 'STATUSNOTANET'
        DataSource = DM.dsParametros
        TabOrder = 8
        ValueChecked = 'T'
        ValueUnchecked = 'F'
      end
      object edtarquivo: TFilenameEdit
        Left = 24
        Top = 320
        Width = 289
        Height = 21
        NumGlyphs = 1
        TabOrder = 9
        OnExit = edtarquivoExit
      end
    end
    object TabSheet3: TTabSheet
      Caption = 'Textos'
      ImageIndex = 2
      object Label30: TLabel
        Left = 11
        Top = 123
        Width = 111
        Height = 13
        Caption = 'Texto / Histórico'
        FocusControl = DBEdit14
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label31: TLabel
        Left = 11
        Top = 6
        Width = 144
        Height = 13
        Caption = 'Texto / Requerimento'
        FocusControl = DBEdit14
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label32: TLabel
        Left = 11
        Top = 221
        Width = 124
        Height = 13
        Caption = 'Texto / Certificado'
        FocusControl = DBEdit14
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBMemo1: TDBMemo
        Left = 8
        Top = 137
        Width = 557
        Height = 77
        DataField = 'TEXTO_HISTORICO'
        DataSource = DM.dsParametros
        Enabled = False
        ScrollBars = ssVertical
        TabOrder = 1
      end
      object DBMemo2: TDBMemo
        Left = 8
        Top = 20
        Width = 557
        Height = 97
        DataField = 'TEXTO_REQUERIMENTO'
        DataSource = DM.dsParametros
        Enabled = False
        ScrollBars = ssVertical
        TabOrder = 0
      end
      object DBMemo3: TDBMemo
        Left = 8
        Top = 235
        Width = 557
        Height = 74
        DataField = 'TEXTO_CERTIFICADO'
        DataSource = DM.dsParametros
        Enabled = False
        ScrollBars = ssVertical
        TabOrder = 2
      end
    end
  end
  object BitBtn1: TBitBtn
    Left = 496
    Top = 476
    Width = 75
    Height = 25
    Caption = '&Fecha'
    TabOrder = 1
    Kind = bkClose
    Layout = blGlyphRight
  end
  object Disci: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCADISCI.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 632
    Top = 24
    object DisciDIS_COD: TSmallintField
      FieldName = 'DIS_COD'
    end
    object DisciDIS_NOME: TStringField
      FieldName = 'DIS_NOME'
      Size = 40
    end
    object DisciDIS_RESUM: TStringField
      FieldName = 'DIS_RESUM'
      Size = 4
    end
    object DisciDIS_MAXFAL: TFloatField
      FieldName = 'DIS_MAXFAL'
    end
  end
  object Grade: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCAGRADE.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 632
    Top = 120
    object GradeCUR_COD: TSmallintField
      FieldName = 'CUR_COD'
    end
    object GradeSER_NUMERO: TSmallintField
      FieldName = 'SER_NUMERO'
    end
    object GradeGRA_SEQ: TSmallintField
      FieldName = 'GRA_SEQ'
    end
    object GradeDIS_COD: TSmallintField
      FieldName = 'DIS_COD'
    end
    object GradeGRA_CHSEM: TSmallintField
      FieldName = 'GRA_CHSEM'
    end
    object GradeGRA_CHOF: TSmallintField
      FieldName = 'GRA_CHOF'
    end
    object GradeGRA_CHNO: TSmallintField
      FieldName = 'GRA_CHNO'
    end
    object GradeGRA_TIPO: TStringField
      FieldName = 'GRA_TIPO'
      Size = 2
    end
  end
  object Mapa: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCAMAPA.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 632
    Top = 168
    object MapaCUR_COD: TSmallintField
      FieldName = 'CUR_COD'
    end
    object MapaSER_NUMERO: TSmallintField
      FieldName = 'SER_NUMERO'
    end
    object MapaTUR_TURMA: TStringField
      FieldName = 'TUR_TURMA'
      Size = 2
    end
    object MapaTUR_ANOLET: TSmallintField
      FieldName = 'TUR_ANOLET'
    end
    object MapaMAP_DIASEM: TSmallintField
      FieldName = 'MAP_DIASEM'
    end
    object MapaMAP_AULA: TSmallintField
      FieldName = 'MAP_AULA'
    end
    object MapaDIS_COD: TSmallintField
      FieldName = 'DIS_COD'
    end
  end
  object Turma: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCATURMA.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 632
    Top = 72
    object TurmaCUR_COD: TSmallintField
      FieldName = 'CUR_COD'
    end
    object TurmaSER_NUMERO: TSmallintField
      FieldName = 'SER_NUMERO'
    end
    object TurmaTUR_TURMA: TStringField
      FieldName = 'TUR_TURMA'
      Size = 2
    end
    object TurmaTUR_ANOLET: TSmallintField
      FieldName = 'TUR_ANOLET'
    end
    object TurmaTUR_PERIOD: TStringField
      FieldName = 'TUR_PERIOD'
      Size = 1
    end
    object TurmaTUR_VAGAS: TSmallintField
      FieldName = 'TUR_VAGAS'
    end
  end
  object Nota: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCANOTAS.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 632
    Top = 216
    object NotaALU_COD: TIntegerField
      FieldName = 'ALU_COD'
    end
    object NotaDIS_COD: TSmallintField
      FieldName = 'DIS_COD'
    end
    object NotaBIM_NUMERO: TStringField
      FieldName = 'BIM_NUMERO'
      Size = 1
    end
    object NotaNOT_NOTA: TFloatField
      FieldName = 'NOT_NOTA'
    end
  end
  object Faltas: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCAFALTA.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 664
    Top = 312
    object FaltasALU_COD: TIntegerField
      FieldName = 'ALU_COD'
    end
    object FaltasDIS_COD: TSmallintField
      FieldName = 'DIS_COD'
    end
    object FaltasFAL_DATA: TDateField
      FieldName = 'FAL_DATA'
    end
    object FaltasFAL_QTDE: TSmallintField
      FieldName = 'FAL_QTDE'
    end
  end
  object IBSQL1: TIBSQL
    Database = DM.DB_ANGLO
    ParamCheck = True
    Transaction = DM.IBTr_ANGLO
    Left = 608
    Top = 264
  end
  object Recup: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCARECUP.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 664
    Top = 264
    object RecupALU_COD: TIntegerField
      FieldName = 'ALU_COD'
    end
    object RecupDIS_COD: TSmallintField
      FieldName = 'DIS_COD'
    end
    object RecupBIM_NUMERO: TStringField
      FieldName = 'BIM_NUMERO'
      Size = 1
    end
    object RecupNOT_NOTA: TFloatField
      FieldName = 'NOT_NOTA'
    end
  end
  object Notas: TIBQuery
    Database = DM.DB_ANGLO
    Transaction = DM.IBTr_ANGLO
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from NOTAS'
      'where ALUNO = :aluno and DISCIPLINA = :disciplina and'
      '          BIMESTRE = :bimestre')
    Left = 688
    Top = 112
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'aluno'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'disciplina'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'bimestre'
        ParamType = ptUnknown
      end>
    object NotasALUNO: TIntegerField
      FieldName = 'ALUNO'
      Required = True
    end
    object NotasDISCIPLINA: TIntegerField
      FieldName = 'DISCIPLINA'
      Required = True
    end
    object NotasBIMESTRE: TSmallintField
      FieldName = 'BIMESTRE'
      Required = True
    end
    object NotasNOTA1: TIBBCDField
      FieldName = 'NOTA1'
      Precision = 4
      Size = 1
    end
    object NotasNOTA2: TIBBCDField
      FieldName = 'NOTA2'
      Precision = 4
      Size = 1
    end
    object NotasNOTA3: TIBBCDField
      FieldName = 'NOTA3'
      Precision = 4
      Size = 1
    end
    object NotasNOTA: TIBBCDField
      FieldName = 'NOTA'
      Precision = 4
      Size = 1
    end
    object NotasRECUPBIM: TIBBCDField
      FieldName = 'RECUPBIM'
      Precision = 4
      Size = 1
    end
  end
  object IncFaltaA: TIBStoredProc
    Database = DM.DB_ANGLO
    Transaction = DM.IBTr_ANGLO
    StoredProcName = 'INC_FALTAS_ALUNO'
    Left = 592
    Top = 312
    ParamData = <
      item
        DataType = ftInteger
        Name = 'ALUNO'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'DISCIPLINA'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'DIA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'QTDADE'
        ParamType = ptInput
      end>
  end
  object FS1: TFormStorage
    StoredValues = <>
    Left = 356
    Top = 65520
  end
  object ColorDialog1: TColorDialog
    Ctl3D = True
    Color = clSilver
    Left = 404
    Top = 65528
  end
  object IncDiasLetivos: TIBStoredProc
    Database = DM.DB_ANGLO
    Transaction = DM.IBTr_ANGLO
    StoredProcName = 'INCLUI_DIAS_LETIVOS'
    Left = 584
    Top = 352
    ParamData = <
      item
        DataType = ftDate
        Name = 'DATA'
        ParamType = ptInput
      end>
  end
  object DiasLetivos: THalcyonDataSet
    About = 'Halcyon Version 06.6.0 (21 Mar 00)'
    AutoFlush = False
    DatabaseName = 'C:\SISTEMAS\ANGLO\2001'
    ExactCount = True
    Exclusive = False
    LockProtocol = Default
    TableName = 'SCALETIV.DBF'
    TranslateASCII = True
    UseDeleted = False
    UserID = 0
    Left = 664
    Top = 352
    object DiasLetivosLET_DATA: TDateField
      FieldName = 'LET_DATA'
    end
    object DiasLetivosLET_DESC: TStringField
      FieldName = 'LET_DESC'
      Size = 50
    end
  end
end
