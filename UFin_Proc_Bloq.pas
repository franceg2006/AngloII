...
Begin
    ...
    inicial := agorames;
    if dm.AlunosMESFINAL.IsNull then    //se o mês final for diferente de 12
    final := 12 else
    ...
    begin
      ...
      //***************************************************************************
       ...
       begin
        ...
        3 : begin // se for 3 série
                If (IqCursinhoCursinho.value <> 1) Then  //se não for do cursinho
                Begin
      ...
      for i := inicial to final do //Pega o mês inicia que pode ser 2 ou o mês que o aluno fez a matricula, o mês fim é 12
      begin
              ...
              DM.BloquetosVENCIMENTO.Value := StrToDate(IntToStr(d_dia)+'/'+IntToStr(i)+'/'+Principal.Ano.Caption);  //Para data do vencimento último dia do mês


            Begin
              if (i = 2) then
                DM.BloquetosDATA_LIMITE.Value := EncodeDate(agoraano, i,20) //Data limite para pagamento de pontualidade do mês de fevereiro
                else DM.BloquetosDATA_LIMITE.Value := EncodeDate(agoraano, i,10); //Data limite para pagamento de pontualidade demais meses.
            end;


          if (dm.Bloquetos.State in [dsInsert, dsEdit]) then //Se a tabela de bloqueto estiver no modo de inserção ou edição
          begin
            ...
            DM.BloquetosDESCONTO_ANT.Value := (mensalidade - desconto); //Corresponde ao valor que é dado de desconto, por pagamento antecipado
            ...
            DM.BloquetosST.Value := 'S'; //Coloca a situação ST sento S, para os bloquetos criados
            dm.Alunos.Open;
        end;    // do for - Parcelas
         DM.Bloquetos.Post;
...
//Para mudar o 1 para 10 no fim da alteração no campo BLOQUETO_1SEM ou BLOQUETO_2SEM
        DM.Alunos.Edit;
