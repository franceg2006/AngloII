...
Begin
   ...
   Begin
    ...
    if (dm.AlunosMESINICIAL.IsNull) then
     ...
     inc(cont_adic);
     for i := 1 to total_parcelas do //Pega o mês inicia que pode ser 2 ou o mês que o aluno fez a matricula, o mês fim é 12
     begin
       ...
       inc(cont_parcela);
       d_dia := (DaysPerMonth(agoraano, agorames)); //Quantidade de dia que tem o mês - pega o último dia do mês
...
//       begin
         ...
         DM.BloquetosST.Value := 'G'; //Coloca a situação ST sento G, para os bloquetos gerados
         dm.Alunos.Open;
