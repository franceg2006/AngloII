...
type
  TCad_Semi_Extensivo = class(TForm)
    ...
    procedure CSemestreChange(Sender: TObject);
...
begin
 BitBtn1.SetFocus;
  ...
  Begin
     dm.Semi.Edit;
     dm.SemiUSUARIO.Value := codigouser;
     if CSemestre.ItemIndex = 0 then
      dm.semiCURSO.value := 'SEMI'
     else
      dm.SemiCURSO.Value := 'SEM2';
     dm.Semi.Post;
...
begin
  if rordem.ItemIndex = 0 then
    begin
      ...
      dm.IBQGeral.SQL.Strings[2] := 'where codigo is not null and curso = :scurso';
      if RadioSemi.ItemIndex = 0 then
         dm.IBQGeral.Params[0].Text := 'SEMI'
         else
         dm.IBQGeral.Params[0].Text := 'SEM2';
      dm.IBQGeral.Open;
    end
    else begin
      dm.IBQGeral.Close;
...
begin
  ...
  dm.IBQGeral.Close;
  dm.IBQGeral.SQL.Strings[3] := 'Order By nome';
  dm.IBQGeral.SQL.Strings[2] := 'where codigo is not null';
  dm.IBQGeral.Open;
end;
...
begin
  Try
    ...
    Doc_ReciboSemiQR.ValorExtenso.Valor := dm.ParametrosMENSALIDADE_5.AsCurrency;
    Doc_ReciboSemiQR.qrlblreferencia1.Caption := 'Pagamento da mensalidade no Semi-Extensivo';
    Doc_ReciboSemiQR.qrlblreferencia2.Caption := 'Pagamento da mensalidade no Semi-Extensivo';
  if rgresp.ItemIndex = 0 then
  begin
    Doc_ReciboSemiQR.QRDBText4.DataField := 'NOME';
    Doc_ReciboSemiQR.QRDBText1.DataField := 'RG';
    Doc_ReciboSemiQR.QRDBText7.DataField := 'NOME';
    Doc_ReciboSemiQR.QRDBText6.DataField := 'RG';    
  end;
  if rgresp.ItemIndex = 1 then
  begin
    Doc_ReciboSemiQR.QRDBText4.DataField := 'PAI_NOME';
    Doc_ReciboSemiQR.QRDBText1.DataField := 'RG_PAI';
    Doc_ReciboSemiQR.QRDBText7.DataField := 'PAI_NOME';
    Doc_ReciboSemiQR.QRDBText6.DataField := 'RG_PAI';
  end;
  if rgresp.ItemIndex = 2 then
  begin
    Doc_ReciboSemiQR.QRDBText4.DataField := 'MAE_NOME';
    Doc_ReciboSemiQR.QRDBText1.DataField := 'RG_MAE';
    Doc_ReciboSemiQR.QRDBText7.DataField := 'MAE_NOME';
    Doc_ReciboSemiQR.QRDBText6.DataField := 'RG_MAE';
  end;
...
procedure TCad_Semi_Extensivo.CSemestreChange(Sender: TObject);
begin
  if CSemestre.ItemIndex = 0 then
  begin
     DM.Semi.Close;
     DM.Semi.Params[0].Value := 'SEMI';
     DM.Semi.Open;
  end;

  if CSemestre.ItemIndex = 1 then
   begin
     DM.Semi.Close;
     DM.Semi.Params[0].Value := 'SEM2';
     DM.Semi.Open;
...
procedure TCad_Semi_Extensivo.TabSheet2Show(Sender: TObject);
begin
      dm.IBQGeral.Close;
      dm.IBQGeral.SQL.Strings[3] := 'Order By nome';
      dm.IBQGeral.SQL.Strings[2] := 'where codigo is not null';
...
procedure TCad_Semi_Extensivo.TabSheet3Exit(Sender: TObject);
begin
